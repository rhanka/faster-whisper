...
import { padOrTrim } from './audio';

// Load library (reuse from whisper_bridge)
const libPath = '/home/antoinefa/src/faster-whisper/ts/whisper_bridge/build/libwhisper_bridge.so';
    ...
    public static getMelFilters(sr: number, nFft: number, nMels: number = 128): Float32Array {
        ...
        for (let i = 0; i < nMels; i++) {
            ...
            for (let j = 0; j < numFftBins; j++) {
                const lower = (freqs[i + 1]! - fftfreqs[j]!) / fdiff[i]!;
                const upper = (fftfreqs[j]! - freqs[i + 1]!) / fdiff[i + 1]!;
                const ramp = 1.0 - Math.min(-lower, upper); // equivalent to the python intersection
                const lowerVal = - (- (freqs[i + 1]! - fftfreqs[j]!) / fdiff[i]! ); // equivalent to ramps
